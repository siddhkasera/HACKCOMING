...
document.addEventListener("DOMContentLoaded", () => {
    const loginForm = document.querySelector("#buyer_login");
    const createAccountForm = document.querySelector("#createAcct");

        document.querySelector("#linkCreateAcct").addEventListener("click", e =>{
        e.preventDefault();
    ...
    document.querySelectorAll(".form_input").forEach(inputElement => {
        inputElement.addEventListener("blur", e =>{
            if(e.target.id == "signupUsername" && e.target.value.length < 10 ){
                setInputError(inputElement, "Username must be atleat 10 characters in length");
            }
        });
